...
import { QUERY_GET_ALL_INVOICE_FREETOWN, QUERY_GET_ALL_INVOICE_WAREHOUSE_LONDON } from './Graphql/Query';
import { useQuery } from '@apollo/client';
import Moment from 'react-moment';
import found from "../assets/Image/nofound.jpg"

export default function Freetown({navigation}) {

  const { data, loading } = useQuery(QUERY_GET_ALL_INVOICE_FREETOWN,{
    pollInterval:1000
  })
        ...
        <View style={{ backgroundColor: "#3498DB", height: 200, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 }}>
            ...
            <View style={{ flexDirection: "column", justifyContent: "center", alignItems: "center", marginTop: 70 }}>
                <View style={{ flexDirection: "row", width: "90%" }}>
                    ...
                    <Text style={{ fontSize: 18, fontFamily: "Poppins-SemiBold", color: "#fff", marginLeft: 10 }}>Freetown </Text>
                </View>
            </View>
        </View>
        ...
        <View style={{ flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
         {
            ...
            
           {
             ...
             <>
              ...
              <>
                {
                  data && data.getInvoiceFreetown.filter((obj)=>obj.invoiceNumber.includes(search)).slice().reverse().map(item => {
                      return (
                          <Card style={{ width: "90%", height: 100, marginTop: 10, marginBottom: 10, elevation: 5, borderRadius: 10 }}>
                              <View style={{ flexDirection: "row", width: "100%", justifyContent: "space-between" }}>
                                  <View style={{ width: "50%" }}>
