...
import { QUERY_GET_ALL_INVOICE_FREETOWN, QUERY_GET_ALL_INVOICE_ON_WAY, QUERY_GET_ALL_INVOICE_PICK_UP, QUERY_GET_ALL_INVOICE_WAREHOUSE_LONDON } from './Graphql/Query';
import { useQuery } from '@apollo/client';
import Moment from 'react-moment';
import found from "../assets/Image/nofound.jpg"
import moment from 'moment';

export default function OnWayBooking({navigation}) {

  const { data, loading } = useQuery(QUERY_GET_ALL_INVOICE_ON_WAY,{
    pollInterval:1000
  })
  ...
  useEffect(() => {
    if (data) {
      setFoundValue(data.getInvoiceOnWay)
    }
  }, [data]);
...
const filterByDate = (e) => {
  ...
  const keyword = e;
  if (keyword !== '') {
    const results = data.getInvoiceOnWay.filter((data) => {
        ...
        <View style={{ backgroundColor: "#3498DB", height: 200, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 }}>
            ...
            <View style={{ flexDirection: "column", justifyContent: "center", alignItems: "center", marginTop: 70 }}>
                <View style={{ flexDirection: "row", width: "90%" }}>
                    ...
                    <Text style={{ fontSize: 18, fontFamily: "Poppins-SemiBold", color: "#fff", marginLeft: 10 }}>On Way </Text>
                </View>
            </View>
        </View>
        {
             data && data.getInvoiceOnWay.length === 0 ?
             <></>
                                    ...
                                    <Card style={{ width: "90%", height: 100, marginTop: 10, marginBottom: 10, elevation: 5, borderRadius: 10 }}>
                                        <View style={{ flexDirection: "row", width: "100%", justifyContent: "space-between" }}>
                                            ...
                                            <View style={{ width: "50%" }}>
                                                ...
                                                {
                                                  item.status === "On_Way" ?
                                                  <Text style={{ fontSize: 11, marginLeft: 10, fontFamily: "Poppins-SemiBold", color: "#F1C40F" }}>On Way</Text>
                                                  :
                                                  <Text style={{ fontSize: 11, marginLeft: 10, fontFamily: "Poppins-SemiBold", color: "#F1C40F" }}>On Way</Text>
                                                }
